var BeamMohrView = Backbone.View.extend({
	...
	var theta = 0;
	theta = this.model.get("beam-theta"); // theta input in degrees
	var thetaRad = PrairieGeom.degToRad(theta);
	// ---------------------------------------------------------------------
	// ----------- Getting stress values -----------------------------------
	// ---------------------------------------------------------------------
	// Stress values will be needed when arrowlengthfixed = false
    var xp = Number(this.model.get("beam-xp"));
    var yp = Number(this.model.get("beam-yp"));
	var w=30;
	var Lb=100;
	var b=3;
	var h=30;
	var V = w*(Lb/2-xp);
	var M = w/2*(Lb*xp - xp*xp);
	var Iz = b*Math.pow(h,3)/12;

	var maxsigma = w*Lb*Lb*h/(16*Iz);
    var sigmax = -M*yp/Iz;
    var sigmay = 0;
    var tauxy = ((3*V)/(2*b*h))*(1-Math.pow(2*yp/h,2));

	//console.log("Coordinate = ",xp,yp);
	//console.log("stresses = ",sigmax,sigmay,tauxy);
	// ---------------------------------------------------------------------
	...
	//console.log("Transformed Stresses = ",theta,sigmaxp,sigmayp,tauxyp);
	// ---------------------------------------------------------------------
	...
	// Normalized stresses
	var sx = L*sigmaxp/Math.abs(maxsigma);
	var sy = L*sigmayp/Math.abs(maxsigma);
	var txy =L*tauxyp/Math.abs(maxsigma);
	//console.log("Principal Stresses = ",sigma1,sigma2,thetap1,R);
	//console.log("Normalized Stresses = ", sx, sy, txy);
	...
	if (arrowlengthfixed) {
	    ...
	    var d1 = 1.4*L;
	    var d2 = 2*L*Math.abs(sx)/3;
	    var d3 = d2;
	    var x = 1.2*L;
	    var y = 2*L*Math.abs(txy)/3;
	}
	var rE = $V([OX+d1,OY]);
...
$(function() {
    var beamMohrData = new Backbone.Model({
        'beam-theta': 0,
		'beam-xp': 0,
		'beam-yp': 0,
    });

    rivets.bind($('#beam-container'), beamMohrData)

    var beamMohrView = new BeamMohrView({
        model: beamMohrData,
        el: "beam-rot-elem",
    });
